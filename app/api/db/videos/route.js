...
export async function POST(req) {
  const {url,status,teacherid,character,title,description} = await req.json();
  const pool = await connectToDatabase();
  try {
    await pool.request()
      ...
      .query("INSERT into videos (url,status,teacherid,character,title) VALUES (@url,@status,@teacherid,@character,@title,@description)"
        // ,[name,img,role]
      );
...
export async function PUT(request) {
  ...
  try {
    await pool.request()
      ...
      .query(`UPDATE videos SET ${column} = @data WHERE id = @id`);
